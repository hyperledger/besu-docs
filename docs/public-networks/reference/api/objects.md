...
| `strict`               |         Tag         | Determines if the sender account balance is considered during gas estimation. If `true`, the sender’s balance is checked against the transaction’s gas parameters. This ensures the estimated gas reflects what the sender can actually afford. If `false`, the balance checks are skipped. The default is `true`. When `strict` is `true`, node behavior may also be influenced by the node’s configured [`--estimate-gas-tolerance-ratio`](../cli/options.md#estimate-gas-tolerance-ratio), which controls how precise the estimate needs to be.                                 |
